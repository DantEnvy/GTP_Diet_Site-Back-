...
app.post('/', async (req, res) => { 
    try {
        ...
        if (language == 'uk') {
            promptText = `
РОЛЬ ТА МЕТА:
Дій як клінічний дієтолог. Створи суворо розрахований план харчування на 7 днів на основі наведених нижче біометричних даних.
ВАЖЛИВО: Використовуй тільки ПРОСТІ, звичайні страви (каші, супи, запечене м'ясо, овочі). Жодної екзотики.

МОВНЕ ОБМЕЖЕННЯ:
ВЕСЬ ВИВІД МАЄ БУТИ ВИКЛЮЧНО УКРАЇНСЬКОЮ МОВОЮ.

КРИТИЧНІ ПРАВИЛА ВИВОДУ (СУВОРО ДОТРИМУВАТИСЬ):
1. ЖОДНИХ розмовних вставок, привітань чи вступів. Починати негайно з "## День 1".
2. НЕ повторювати вхідні дані користувача.
3. КАРТИНКА: Вставляй лише ОДНУ картинку на день, суворо ПІСЛЯ заголовка дня.
4. ФОРМАТ КАРТИНКИ: Використовуй посилання: ![image](https://images.unsplash.com/photo-1547592166-23ac45744acd?auto=format&fit=crop&w=800&q=80&sig=[X]), де [X] — це номер дня (1, 2, 3...). Це професійні фото готової їжі, які будуть стабільними.
5. НЕ використовувати таблиці. Використовуй списковий формат.

ЗМІННІ КОРИСТУВАЧА:
- Мета: ${goal || "підтримка ваги"}
- Вік: ${age} | Зріст: ${height}см | Вага: ${weight}кг | Стать: ${gender}
- Алергії: ${allergy} | Стан здоров'я: ${health} | Уникати: ${food}
- ДЕННІ ЦІЛІ: Калорії: ${bmr} ккал | Б: ${protein}г | Ж: ${fat}г | В: ${carb}г
- Мікронутрієнти: ${typeof vitamins === 'object' ? JSON.stringify(vitamins) : vitamins}

ФОРМАТ ВИВОДУ:

## День 1
![image](https://images.unsplash.com/photo-1547592166-23ac45744acd?auto=format&fit=crop&w=800&q=80&sig=1)

**Сніданок:** [Назва простої страви]
- Інгредієнти: [Список з вагою]
- Поживна цінність: [Ккал, Б, Ж, В, Вітаміни]

**Обід:** [Назва простої страви]
...

**Вечеря:** [Назва простої страви]
...

**Перекус:** [Назва простої страви]
...

**Підсумок Дня 1:** Всього Ккал: [X], Білки: [X]г, Жири: [X]г, Вуглеводи: [X]г.

[Повторити для Днів 2-7, змінюючи sig=1 на sig=2, sig=3 і т.д.]
